...
<?php
	...
	mysql_select_db($db_add, $con);
	$result = mysql_query("SELECT time,block,txid,valch FROM ".$tb_add." WHERE address='".$address."'");//2016.9.14由于数据表中存在较多的重复数据,进行唯一性修复
	while($row = mysql_fetch_array($result))
	{
...
<form action="" method="post"> <input type="hidden" name="chaxun" value="查询隐藏的交易记录" />
<input type="submit" name="button" class="btn" value="查询相关交易记录" /> </form>
</div>
