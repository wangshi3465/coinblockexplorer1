...
if (!isset($break) || rpc_error_check(false)) {
	...
	foreach ((array)$block['tx'] as $key => $txid) {
	  ...
	  if (!empty($tx)) {
	    ...
	    if (count($tx['vin']) > 0) {
		  foreach ((array)$tx['vin'] as $key => $value) {
			...
			foreach ((array)$tx_vin['vout'] as $key => $value_vin) {
				    ...
				    //echo "挖矿<br />";
		                ...
		                foreach ($value_vin['scriptPubKey'] as $add_value_vin){
				            ...
				            "</a>:</br><span class='happy_txt'><b>$clean_val_vin</b></span>&nbsp;$curr_code";
							}
		                }
			        }		
		        }
		...
		foreach ((array)$tx['vout'] as $key => $value) {
		  ...
		  if($clean_val == 0) {
			 ...
			 if(is_array($add_value))
			  echo "<a href='./?address=".$add_value[0]."'>".$add_value[0].
				   "</a>:</br><span class='happy_txt'><b>$clean_val</b></span>&nbsp;$curr_code";
		     }
			}
		...
		echo "<a href='./?address=".$add_play_out."'>".$add_play_out.
				   "</a>:</br><span class='happy_txt'><b>$out_total</b></span>&nbsp;$curr_code";
		$fee_name = "挖矿所得";		 
		$dig_flag_out = false;
		$add_play_out = '';
		}
		...
		<b>$fee_name:</b>$fee&nbsp;$curr_code</br></br></td></tr></tr>";
	  }
	}
